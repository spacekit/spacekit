...
class Relay extends EventEmitter {
  ...
  connect () {
    ...
    this.ws.on('open', () => {
      ...
      if (!this.config.noTls) {
        ...
        this.certificate = {
          ensureValidCertificate: () => Promise.resolve(),
          challengeValue: null
        };
      }
  ...
  openSecureConnection (id, port) {
    this.certificate.ensureValidCertificate().then((secureContext) => {
      ...
      let socket = Net.connect(portInfo.destinationPort, portInfo.hostname);
      if (!this.config.noTls) {
        return new TlsTerminatingSocket(socket, secureContext);
      } else {
        return socket;
      }
    }).then((socket) => {
